...
save(error.folds.few, file = "errorfew.rda")
```

```{r}
load("errorfew.rda")

head(error.folds.few, 10)
...
few_model <- rand_forest(min_n = 21,
              ...
              mode = "classification") %>%
  set_engine("ranger") %>%
  fit(koi_disposition ~ koi_period + koi_teq + koi_insol + koi_srad + koi_impact + koi_steff, data = koi_train)
few_model %>%
  predict(koi_test) %>%
  bind_cols(koi_test) %>%
  metrics(truth = koi_disposition, estimate = .pred_class)
...
Still, against this particular model, KNN wins by about 1%.

# Conclusion

We used four models: KNN, Decision Tree, Boosted Trees, and Random Forest. KNN did not work as well as the other three, but nevertheless, still produced results with an accuracy greater than 33%. Since the other three models pushed towards an accuracy of 90% on testing data, we can be quite confident that implementation of our model can result in a more efficient workflow for KOI researchers as they can now prioritize objects predicted to be CANDIDATEs or CONFIRMED.