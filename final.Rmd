...
#plotting koi_dis against koi_period, using this because of the correlation chart #2
koi%>%
  ggplot() +
  aes(x= koi_disposition, y= koi_period, color = koi_disposition)+ geom_boxplot(show.legend = TRUE)

#koi_depth is the degree to which the light of the fixed star is occluded at the moment in the transit where light from the star is least
#equal to the ratio between the planet's radius (R p) and the star's radius (R *) squared

koi%>%
  ggplot() +
  aes(x= koi_disposition, y= koi_depth, color = koi_disposition)+ geom_boxplot(show.legend = TRUE)
